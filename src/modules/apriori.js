...
import { aprioriABI } from '../abi/aprioriABI.js'

export default async function apriori(provider, wallet, contract, channel) {
  const iface = new Interface(aprioriABI)
  ...
  for (let i = 0; i < config.cycle; i++) {
    ...
    for (let retry = 0; retry < config.retry; retry++) {
      ...
      try {
        ...
        await channel.send(`STAKE ${amountStr} MON → aprMON → TX [${short}](<${config.explorer}${tx.hash}>)`)
        stakeTx = tx
    ...
    for (let retry = 0; retry < config.retry; retry++) {
      ...
      try {
        const tx = await wallet.sendTransaction({
          to: contract.contract,
          data: iface.encodeFunctionData('requestRedeem', [amount, wallet.address, wallet.address]),
          gasLimit
        })
        ...
        const short = tx.hash.slice(0, 7) + '...' + tx.hash.slice(-7)
        await channel.send(`UNSTAKE ${amountStr} aprMON → MON → TX [${short}](<${config.explorer}${tx.hash}>)`)
        unstakeTx = tx
        if (config.rawInfo) await rawInfo(provider, tx.hash, channel, iface)
        break
      } catch (err) {
