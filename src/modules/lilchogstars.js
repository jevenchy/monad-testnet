...
import { lilchogstarsABI } from '../abi/lilchogstarsABI.js'

export default async function lilchogstars(provider, wallet, contract, channel) {
  const minter = new Contract(contract.contract, lilchogstarsABI, wallet)
  ...
  for (let i = 0; i < config.cycle; i++) {
    ...
    for (let retry = 0; retry < config.retry; retry++) {
      ...
      try {
        const tx = await minter.mint(1, { gasLimit })
        ...
        await channel.send(`Minted OPEN EDITION CHOGSTAR → TX [${short}](<${config.explorer}${tx.hash}>)`)
        mintTx = tx
