...
import { moncockABI } from '../abi/moncockABI.js'

export default async function moncock(provider, wallet, contract, channel) {
  const minter = new Contract(contract.contract, moncockABI, wallet)
  ...
  for (let i = 0; i < config.cycle; i++) {
    ...
    for (let retry = 0; retry < config.retry; retry++) {
      ...
      try {
        ...
        await channel.send(`Minted Moncock OG → TX [${short}](<${config.explorer}${tx.hash}>)`)
        mintTx = tx
