...
import { octoABI } from '../abi/octoABI.js'

export default async function octo(provider, wallet, contract, channel) {
  const dex = new Contract(contract.router, octoABI, wallet)
  ...
  for (let i = 0; i < config.cycle; i++) {
    ...
    for (const token of tokens) {
      ...
      const tokenContract = new Contract(token.address, octoABI, wallet)
