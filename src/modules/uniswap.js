...
export default async function uniswap(provider, wallet, contract, channel) {
  const dex = new Contract(contract.router, uniswapABI, wallet)
  ...
  for (let i = 0; i < config.cycle; i++) {
    ...
    for (const [symbol, { address }] of allTokens) {
      ...
      const token = new Contract(address, uniswapABI, wallet)
